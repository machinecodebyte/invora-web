...
const READY_FIXTURE: ForecastResultsE2EFixture = {
  ...
  productResults: [FORECAST_PRODUCT_RESULT],
};
...
test.describe('Forecast Results', () => {
  test('redirects unauthenticated visitors before result content is exposed', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Forecast Results' }),
    ).not.toBeVisible();
  });

  test('shows a safe no-selection state for an authenticated user', async ({
    page,
  }) => {
    await signIn(page);
    ...
    await expect(
      page.getByText('Select a forecast run to view results.'),
    ).toBeVisible();
  });

  test('renders summary, backend metrics, prediction rows, zero values, and nullable actuals', async ({
    page,
  }) => {
    await visitResults(page);

    await expect(
      page.getByRole('heading', { level: 1, name: 'Forecast Results' }),
    ).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Forecast summary' })).toBeVisible();
    await expect(
      page
        .getByRole('table', { name: 'Forecast predictions' })
        .getByText('Zero-demand Widget')
        .first(),
    ).toBeVisible();
    await expect(page.getByText('MAPE')).toBeVisible();
    await expect(
      page.getByText('Actual data is unavailable for 1 period.'),
    ).toBeVisible();
    await expect(
      page.getByRole('img', {
    ...
    ).not.toContainText('Actual');
  });

  test('filters prediction rows by a backend-supported SKU search', async ({
    page,
  }) => {
    await visitResults(page);
  ...
  test('opens a product-level Forecast Result from an existing prediction row', async ({
    page,
  }) => {
    await visitResults(page);
    await page
      .getByRole('button', { name: 'View forecast detail for Zero-demand Widget' })
      .first()
      .click();
    ...
    await expect(page.getByRole('dialog')).toContainText('Total predicted demand');
    await expect(
      page.getByRole('table', { name: 'Product forecast detail' }),
    ).toBeVisible();
  });

  test('validates an inverted forecast date range without querying results', async ({
    page,
  }) => {
    await visitResults(page);
  ...
  test('renders an honest completed-run empty state', async ({ page }) => {
    await visitResults(
      page,
      { state: 'ready', data: EMPTY_FORECAST_RESULTS_DATA },
      EMPTY_FORECAST_RESULTS_DATA.overview.runId,
    );
  ...
  test('renders a safe not-ready and failed-run state', async ({ page }) => {
    ...
    await expect(
      page.getByText('Forecast results are not available yet.'),
    ).toBeVisible();

    await setForecastResultsFixture(page, { state: 'failed_run' });
    await page.goto(`/forecasts/results?runId=${FORECAST_RESULTS_RUN_ID}`);
    await expect(
      page.getByText('Forecast results are unavailable because the run failed.'),
    ).toBeVisible();
  });

  test('renders a safe service error without implementation detail', async ({
    page,
  }) => {
    await visitResults(page, { state: 'error' });
    await expect(
      page.getByText('Unable to load Forecast Results.', { exact: true }),
    ).toBeVisible();
